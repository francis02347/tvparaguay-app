...
13. REPLACING DEFECTIVE STREAMING LINKS & HANDLING EMBEDDED PLAYERS:
    - If the user asks to fix/replace a defective channel (e.g. Unicanal, Telefuturo, Trece, Gen, etc.), search the web for M3U8 links, but also inspect their official websites (e.g. unicanal.com.py/en-vivo, trece.com.py/en-vivo, gen.com.py/live/) for embedded players (especially Dailymotion iframe URLs or platform tokens).
    - If you find a Dailymotion video ID from an official site, the channel can be resolved by the app using our native Dailymotion bypass. You MUST write it in ChannelData.java as:
      `dailymotion://<videoId>?referer=<official_site_domain_with_slash>&embedder=<official_site_domain_with_slash>`
      (e.g., `dailymotion://k4nLYiNrBX8W5jDbSlM?referer=https://trece.com.py/&embedder=https://trece.com.py/` or `dailymotion://k1mHLKycOlKgo3Db5GI?referer=https://www.unicanal.com.py/&embedder=https://www.unicanal.com.py/`).
    - DYNAMIC TOKENS (desdeparaguay.net): If a channel hosted on desdeparaguay.net (like Gen) requires dynamic tokens (k and exp) that expire quickly, you can use our dynamic resolver in PlayerActivity.java by setting the URL in ChannelData.java as:
      `desdeparaguay://<channel_id>`
      (e.g., `desdeparaguay://gentv` for Gen, which will fetch the player at gentv.desdepylabs.com and extract the live URL with updated tokens at runtime).
    - DEDUPLICATION & NAME COLLISIONS: If you add or update a hardcoded channel (e.g., "Trece"), check if `default_channels.m3u` in assets contains an international channel with a similar name and quality suffix (like "Trece (1080p)"). The app's ChannelDeduplicator will normalize both to the same name and discard the lower-quality or hardcoded one. You MUST rename the international duplicate in `default_channels.m3u` (e.g., change "Trece (1080p)" to "Trece España (1080p)") to prevent the deduplicator from overriding the local channel.
    - Always test candidate streams via a short Python script. If the CDN stream returns only audio (typically containing codec mp4a.40.2 and no video codec), discard it and search for the official web embed stream. Once fixed, update ChannelData.java, commit the changes, compile with `powershell -File actualizar.ps1 -Auto -ReleaseNotes "Notes..."`, and send/publish the updated APK.
"""
    }]
}
