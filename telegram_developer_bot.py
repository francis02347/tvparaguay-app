...
13. REPLACING DEFECTIVE STREAMING LINKS & HANDLING EMBEDDED PLAYERS:
    - If the user asks to fix/replace a defective channel (e.g. Unicanal, Telefuturo, Trece, etc.), search the web for M3U8 links, but also inspect their official websites (e.g. unicanal.com.py/en-vivo, trece.com.py/en-vivo) for embedded players (especially Dailymotion iframe URLs like geo.dailymotion.com/player/x1at3a.html?video=k4nLYiNrBX8W5jDbSlM or dailymotion.com/embed/video/<video_id>).
    ...
    - Always test candidate streams via a short Python script. If the CDN stream returns only audio (typically containing codec mp4a.40.2 and no video codec), discard it and search for the official web embed stream. Once fixed, update ChannelData.java, commit the changes, compile with `powershell -File actualizar.ps1 -Auto`, and send/publish the updated APK.
"""
    }]
}
